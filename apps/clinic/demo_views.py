...
def contact_submit(request):
    """Форма «зв'яжіться зі мною» з лендінгу: ім'я + телефон + пошта обов'язкові.
    Пишемо ContactLead(notified=false) — далі сторож ядра забере й повідомить Олега."""
    ip = _client_ip(request)

    rl_key = f"contact-rl:{ip}"
    count = cache.get(rl_key, 0)
    if count >= 5:
        return JsonResponse(
            {"ok": False, "error": "Забагато заявок. Спробуйте трохи згодом."},
            status=429,
        )
    ...
    if len(name) < 2:
        return JsonResponse(
            {"ok": False, "error": "Вкажіть, як до вас звертатись."}, status=400)
    if not _PHONE_RE.match(phone):
        return JsonResponse(
            {"ok": False, "error": "Телефон у форматі +380XXXXXXXXX або 0XXXXXXXXX"},
            status=400)
    try:
        validate_email(email)
    except ValidationError:
        return JsonResponse(
            {"ok": False, "error": "Перевірте адресу електронної пошти."}, status=400)
